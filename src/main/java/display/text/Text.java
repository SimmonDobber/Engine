    ...
    private void mergeSymbolIntoPixelArray(int[] pixels, Rasterable symbol, int currentX, int currentY) {
        ...
        for (int x = 0; x < symbol.getW(); x++) {
            for (int y = 0; y < symbol.getH(); y++) {
                int symbolPixelValue = symbolPixels[x + y * symbol.getW()];
                int pixelId = currentX + x + (currentY + y) * this.w;
                pixels[pixelId] = (symbolPixelValue != getTransparentColorValue() ? this.color.getValue() : symbolPixelValue);
            }
        }
    }
