...
import display.DrawableComposition;
...
import static input.InputBean.getInput;
...
public class TextField implements Visual, Interactive, InputObserver {
    ...
    public TextField(int x, int y, int w, int h, int textOffsetX, int textOffsetY, String backgroundColor, String font, String fontColor, boolean masked, int limit) {
        this.background = DisplayBean.getDisplay().getDrawableFactory().makeRectangle(x, y, w, h, backgroundColor);
        this.text = DisplayBean.getDisplay().getDrawableFactory().makeText("", x + textOffsetX, y + textOffsetY, font, fontColor);
